...
def get_feedback(user_id):
    ...
    cur = con.cursor()
    cur.execute("SELECT title, message FROM card WHERE user_id=? ", (user_id,))
    return cur.fetchall()
    


def load_user(user_id):
    """Return a User instance by id or None."""
    if not user_id:
        return None
...
def feed():
    if request.method == 'POST':
        ...
        card = Card(title=title, message=message, user_id=user_id)
        db.session.add(card)
        db.session.commit()
        return redirect('/site')
    else:
        user_id = session.get('user_id')
        cards = Card(get_feedback(user_id))  
